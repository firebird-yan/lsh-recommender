...
class UPCCRecommender(Recommender):
    ...
    def predictData(self, active_user, ws):
        ...
        for i in range(top):
            #计算相似用户可用服务的平均值
            if self.sample_indices[similar_users[i]][ws] == 1:
                cur_value = self.data[similar_users[i]][ws]
                if cur_value != -1:
                    total_similarities += similarities[i]
                    avg_value_of_similar = np.average(self.data[similar_users[i]][indices == 1])
                    predicted_value += similarities[i] * (cur_value - avg_value_of_similar)
                    
        
        if total_similarities > 0.0:
            predicted_value /= total_similarities
    ...
    def findSimilarUsers(self, index, top):
        similarities = []
    ...
    def calculateSimilarity(self, test_index, train_index):
        ...
        product = self.sample_indices[train_index] * \
            self.sample_indices[test_index]
        
        test = self.processed_data[test_index][product == 1]
        train = self.processed_data[train_index][product == 1]
